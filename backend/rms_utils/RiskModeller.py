...
def groupUpload(Job_Id,authFile,ccy,reins=False,pathToFolder=None):
    ...
    if (pathToFolder==None):
        accFile=getACCfile(Job_Id,ccy)
        locFile=getLOCfile(Job_Id,ccy)
        if (reins==True):
...
def getCurrentDLM():
    ...
    df=pd.read_csv("DLM List-current.csv")
...
def DLMjobSubPort_2(portID,edmDataSource,authFile,ccy="USD",modelProfileId=1,eventRateSchemeId=0,outputProfile="PORT_ACC_POL_LOC( PF_V1)"):
    ...
    ##progressBar(workFlowID,authFile)
    time.sleep(5)
    print("Job Submission is finished")
    return workFlowID
...
#Each output profile require a set of Metric requests
...
def getACCfile(Job_Id,ccy,pathToFolder=None):
    if (pathToFolder==None):
        pathToFolder='./outputs/' + Job_Id
    print(Job_Id.split("-")[0])
    print(type(ccy))
...
def getLOCfile(Job_Id,ccy,pathToFolder=None):
    if (pathToFolder==None):
        pathToFolder='./outputs/' + Job_Id
...
def getCatOutput1(EDMname,authFile,Style='Default'):
    url = "https://pf-cat-modelling-results.azurewebsites.net/results?datasource="+EDMname
...
def getCatOutput2(EDMname,authFile,Style='Default'):
    ...
    #'datasource'
    url = "https://pf-cat-modelling-results.azurewebsites.net/rdm/initiate_download?datasource="+EDMname

    payload={}
    headers = {'Authorization': bearToken}
    response = requests.request("POST", url, headers=headers, data = json.dumps(payload))
    print(response.text)
    print(response.json()[ "jobIDs"])

    print("Set to remove repeatition")
    jobListA= list(set(response.json()[ "jobIDs"]))
    print(jobListA)
    jobList=[]
    while len(jobListA)>0:
        for a in jobListA:
            ...
            print(a)
            currUrl="https://pf-cat-modelling-results.azurewebsites.net/get_download?jobID="+str(a)+"&to_riskstore=True&datasource="+EDMname
            headers_logic = {'Authorization': bearToken}
            response_logic = requests.request("GET", currUrl, headers=headers_logic)
            print(response_logic.status_code)
            if response_logic.status_code==200:
#                 print(currUrl)
                jobListA.remove(a)
                print(jobListA)
        #Call the Logic App
    url_logic="https://prod-30.uksouth.logic.azure.com:443/workflows/6c5a28fc7170432597d36d8460ea09d2/triggers/manual/paths/invoke?api-version=2016-10-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=QUBw85lKy5Y5Nu6-8UjpmOZNwgRGSZPiIrSgYQfmoUs"
#     url_logic="https://prod-00.uksouth.logic.azure.com:443/workflows/c0ab4070996f42a28af307b5970c9488/triggers/manual/paths/invoke?api-version=2016-10-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=JnOjBWoFGlEZxHVMqTfMTXEbrThZKS09WRiuPdqgKy0"
    payload_logic={"datasource":EDMname}
    headers_logic = {'Content-Type': 'application/json'}
    response = requests.request("POST", url_logic, headers=headers_logic, data=json.dumps(payload_logic))
    # print(response)
    return response
...
#ONE CLICK
############
def jobAutoSub(auth_file,job_id,team_name="ANA",exe_level="DLM",ccy="USD",reSubmit=False):
    ...
    portInfo=createPortfolio(edm_name,auth_file)
    print(portInfo)
...
def POSTCODEEDITOR(datasource,bearerToken,oldValue,newValue):
    ...
    for i in range(0,rangeMax):
        offset=i*1000
        path =  path = 'locations?datasource=' + datasource +'&limit=1000000000&offset='+str(offset)
